package algorithms.search;
import algorithms.mazeGenerators.Maze;
import algorithms.mazeGenerators.Position;

import java.util.ArrayList;

public class SearchableMaze implements ISearchable{
    private Maze maze;

    public ArrayList<AState> getAllPossibleStates(AState state) {
        ArrayList<AState> maze_state_arr = new ArrayList<AState>();
        boolean flag_up = false;
        boolean flag_down = false;
        boolean flag_left = false;
        boolean flag_right = false;
        for(int i=0;i<8;i++){
            maze_state_arr.add(new MazeState(new Position(-1,-1)));
        }


        if(((Position)state.getState()).getRow() - 1 >= 0)
        {
            Position p = new Position(((Position)state.getState()).getRow() - 1, ((Position)state.getState()).getColumn());
            maze_state_arr.set(0,new MazeState(p));
            flag_up = true;
        } // upper

        if(((Position)state.getState()).getColumn() + 1 < this.maze.getMax_columns())
        {
            Position p = new Position(((Position)state.getState()).getRow(), ((Position)state.getState()).getColumn() + 1);
            maze_state_arr.set(2,new MazeState(p));
            flag_right = true;
        } // right


        if(((Position)state.getState()).getRow() + 1 < this.maze.getMax_rows())
        {
            Position p = new Position(((Position)state.getState()).getRow() + 1, ((Position)state.getState()).getColumn()); // lower
            maze_state_arr.set(4,new MazeState(p));
            flag_down = true;
        }

        if(((Position)state.getState()).getColumn() - 1 >= 0)
        {
            Position p  = new Position(((Position)state.getState()).getRow(), ((Position)state.getState()).getColumn()- 1); // left
            maze_state_arr.set(6,new MazeState(p));
            flag_left = true;
        }

        if(((Position)state.getState()).getRow() - 1 >= 0 && ((Position)state.getState()).getColumn() + 1 < this.maze.getMax_columns()){
            if(flag_up == true || flag_right == true){
                Position p = new Position(((Position)state.getState()).getRow() - 1, ((Position)state.getState()).getColumn() + 1);
                maze_state_arr.set(1,new MazeState(p));
            }
        } // upper right

        if(((Position)state.getState()).getRow() + 1 < this.maze.getMax_rows() && ((Position)state.getState()).getColumn() + 1 < this.maze.getMax_columns()){
            if(flag_down == true || flag_right == true){
                Position p = new Position(((Position)state.getState()).getRow() + 1, ((Position)state.getState()).getColumn() + 1);
                maze_state_arr.set(3,new MazeState(p));
            }
        } // lower right

        if(((Position)state.getState()).getRow() + 1 < this.maze.getMax_rows() && ((Position)state.getState()).getColumn() - 1 >= 0){
            if(flag_down == true || flag_left == true){
                Position p = new Position(((Position)state.getState()).getRow() + 1, ((Position)state.getState()).getColumn() - 1);
                maze_state_arr.set(5,new MazeState(p));
            }
        } // lower left

        if(((Position)state.getState()).getRow() - 1 >= 0 && ((Position)state.getState()).getColumn() - 1 >= 0){
            if(flag_up == true || flag_left == true){
                Position p = new Position(((Position)state.getState()).getRow() - 1, ((Position)state.getState()).getColumn() - 1);
                maze_state_arr.set(7,new MazeState(p));
            }
        } // upper left


        return maze_state_arr;
    }
}
